...
namespace marscat.BLM.Skill;
//100级特化
public class GCD100 : ISlotResolver
{
    public int Check()
    {
        uint 即刻 = 7561;
    ...
    /*private Spell 新玩意()
    {
        ...
        if (火状态)
        {
            ...
            if (火层数 == 3)
            {
                //如果失去标改火苗，则进入双星灵短来止损（星灵-冰4-资源-星灵-悖论-绝望）
                //火阶段收尾，没有火苗，有资源进入短循环并且可以有窗口插入星灵
                if (MP == 0 && !火苗 && 资源数量 >= 1 && GCDHelper.GetGCDCooldown() > 1000)
                {
                    return 获取火苗短循环();
                }
